...
**Creating Edges:**
- Click and drag from a block's output port (right side)
- Release on another block's input port (left side)
- The connection line shows the conversation path

**Types of Edges:**
- **Success Edge**: Default path when a block completes successfully
- **Error Edge**: Path taken when a block encounters an error
- **Conditional Edges**: Paths based on specific conditions (used with Filter blocks)

**Edge Configuration:**
- Click on any edge to add conditions or labels
- Set priority for multiple edges from the same block
- Add custom labels for better flow readability
...
2. **Accessing Variables**:
   - Reference in Talk blocks: `{{variable_name}}`
   - Use in conditions: `{{variable_name}} == "value"`
